...
foreach ($base in $baseList) {
	...
	foreach ($in in $inList) {
		...
		$inValue = $in[1];
		$inValueList = ""
		for ($i0 = 0; $i0 -lt 12; $i0++) { $inValueList += ",$inValue"; }
		foreach ($mid in $midList) {
			...
			foreach ($out in $outList) {
				...
				$outValue = $out[1];
				$outValueList = ""
				for ($o0 = 0; $o0 -lt 12; $o0++) { $outValueList += ",$outValue"; }
...
foreach ($base in $baseList) {
	...
	foreach ($in0 in $inList) {
		...
		$in0Value = $in0[1];
		$in0ValueList = ""
		for ($i0 = 0; $i0 -lt 6; $i0++) { $in0ValueList += ",$in0Value"; }
		foreach ($in1 in $inList) {
			$in1Char = $in1[0];
			$in1Value = $in1[1];
			$in1ValueList = ""
			for ($i1 = 0; $i1 -lt 6; $i1++) { $in1ValueList += ",$in1Value"; }
			foreach ($mid in $midList) {
				$midChar = $mid[0];
				$midValue = $mid[1];
				foreach ($out0 in $outList) {
					...
					$out0Value = $out0[1];
					$out0ValueList = ""
					for ($o0 = 0; $o0 -lt 6; $o0++) { $out0ValueList += ",$out0Value"; }
					foreach ($out1 in $outList) {
						$out1Char = $out1[0];
						$out1Value = $out1[1];
						$out1ValueList = ""
						for ($o1 = 0; $o1 -lt 6; $o1++) { $out1ValueList += ",$out1Value"; }
...
foreach ($base in $baseList) {
	...
	foreach ($in0 in $inList) {
		...
		$in0Value = $in0[1];
		$in0ValueList = ""
		for ($i0 = 0; $i0 -lt 4; $i0++) { $in0ValueList += ",$in0Value"; }
		foreach ($in1 in $inList) {
			$in1Char = $in1[0];
			$in1Value = $in1[1];
			$in1ValueList = ""
			for ($i1 = 0; $i1 -lt 4; $i1++) { $in1ValueList += ",$in1Value"; }
			foreach ($in2 in $inList) {
				$in2Char = $in2[0];
				$in2Value = $in2[1];
				$in2ValueList = ""
				for ($i2 = 0; $i2 -lt 4; $i2++) { $in2ValueList += ",$in2Value"; }
				foreach ($mid in $midList) {
					$midChar = $mid[0];
					$midValue = $mid[1];
					foreach ($out0 in $outList) {
						...
						$out0Value = $out0[1];
						$out0ValueList = ""
						for ($o0 = 0; $o0 -lt 4; $o0++) { $out0ValueList += ",$out0Value"; }
						foreach ($out1 in $outList) {
							$out1Char = $out1[0];
							$out1Value = $out1[1];
							$out1ValueList = ""
							for ($o1 = 0; $o1 -lt 4; $o1++) { $out1ValueList += ",$out1Value"; }
							foreach ($out2 in $outList) {
								$out2Char = $out2[0];
								$out2Value = $out2[1];
								$out2ValueList = ""
								for ($o2 = 0; $o2 -lt 4; $o2++) { $out2ValueList += ",$out2Value"; }
...
foreach ($base in $baseList) {
	...
	foreach ($in0 in $inList) {
		...
		$in0Value = $in0[1];
		$in0ValueList = ""
		for ($i0 = 0; $i0 -lt 3; $i0++) { $in0ValueList += ",$in0Value"; }
		foreach ($in1 in $inList) {
			$in1Char = $in1[0];
			$in1Value = $in1[1];
			$in1ValueList = ""
			for ($i1 = 0; $i1 -lt 3; $i1++) { $in1ValueList += ",$in1Value"; }
			foreach ($in2 in $inList) {
				$in2Char = $in2[0];
				$in2Value = $in2[1];
				$in2ValueList = ""
				for ($i2 = 0; $i2 -lt 3; $i2++) { $in2ValueList += ",$in2Value"; }
				foreach ($in3 in $inList) {
					$in3Char = $in3[0];
					$in3Value = $in3[1];
					$in3ValueList = ""
					for ($i3 = 0; $i3 -lt 3; $i3++) { $in3ValueList += ",$in3Value"; }
					foreach ($mid in $midList) {
						$midChar = $mid[0];
						$midValue = $mid[1];
						foreach ($out0 in $outList) {
							...
							$out0Value = $out0[1];
							$out0ValueList = ""
							for ($o0 = 0; $o0 -lt 3; $o0++) { $out0ValueList += ",$out0Value"; }
							foreach ($out1 in $outList) {
								$out1Char = $out1[0];
								$out1Value = $out1[1];
								$out1ValueList = ""
								for ($o1 = 0; $o1 -lt 3; $o1++) { $out1ValueList += ",$out1Value"; }
								foreach ($out2 in $outList) {
									$out2Char = $out2[0];
									$out2Value = $out2[1];
									$out2ValueList = ""
									for ($o2 = 0; $o2 -lt 3; $o2++) { $out2ValueList += ",$out2Value"; }
									foreach ($out3 in $outList) {
										$out3Char = $out3[0];
										$out3Value = $out3[1];
										$out3ValueList = ""
										for ($o3 = 0; $o3 -lt 3; $o3++) { $out3ValueList += ",$out3Value"; }
